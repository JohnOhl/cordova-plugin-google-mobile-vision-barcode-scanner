...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
  ...
  NSArray<GMVBarcodeFeature *> *barcodes = [self.barcodeDetector featuresInImage: image options:nil];

  //If no barcodes were detected then rotate the image 90 degrees.
  // There is an issue with the barcode scanner not being able to scan data matrix in 2 orientations and rotation introduces at least one orientation to the mix.
  if ([barcodes count] == 0) {
    UIImage *rotated = [UIImage imageWithCGImage:image.CGImage scale:1.0 orientation: UIImageOrientationRight];
    barcodes = [self.barcodeDetector featuresInImage: rotated options:nil];
  }
